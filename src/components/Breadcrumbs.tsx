...
export function Breadcrumbs({ onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  return (
    <div className="shrink-0 border-b border-(--border) bg-(--bg-base) py-3 px-4 sm:px-6">
      <div className="h-7 flex items-center gap-3">
        {/* Hamburger button — mobile only, stays fixed */}
        <button
          onClick={onMenuOpen}
          className="sm:hidden p-1 -ml-1 rounded-lg text-(--text-dim) hover:text-(--text) hover:bg-(--bg-subtle)/60 transition-colors shrink-0"
              ...
              {crumbs.map((crumb, i) => {
                ...
                return (
                  <span key={i} className="flex items-center gap-1.5">
                    ...
                    {crumb.to && !isLast ? (
                      <Link to={crumb.to} className="text-(--text-dim) hover:text-(--text) transition-colors">
                        {crumb.label}
                      </Link>
                    ) : (
                      <span className={isLast ? 'text-(--text)' : 'text-(--text-dim)'}>{crumb.label}</span>
                    )}
                  </span>
                );
              })}
