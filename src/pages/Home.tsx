...
function StatCard({ label, value, color = 'text-[var(--text)]' }: {
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-(--bg-surface) shadow-xs ring-1 ring-(--border) w-full flex flex-col">
      <header className="flex items-center gap-2 bg-(--bg-base) border-b border-(--border) h-14 px-4">
...
function SplitStatCard({ leftLabel, leftValue, leftColor, rightLabel, rightValue, rightColor }: {
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-(--bg-surface) shadow-xs ring-1 ring-(--border) w-full flex flex-col">
      <header className="flex items-center gap-2 bg-(--bg-base) border-b border-(--border) h-14 px-4">
        <span className="truncate text-sm font-medium text-(--text)">{t('home.connectionStatus')}</span>
      </header>
      <div className="flex flex-1">
        <div className="flex-1 px-4 py-4 flex flex-col justify-center min-h-22 border-r border-(--border)">
          <div className="text-xs font-medium text-(--text-muted) mb-1">{leftLabel}</div>
          <span className={`text-2xl font-semibold tabular-nums ${leftColor ?? 'text-(--text)'}`}>{leftValue}</span>
        </div>
        <div className="flex-1 px-4 py-4 flex flex-col justify-center min-h-22">
          <div className="text-xs font-medium text-(--text-muted) mb-1">{rightLabel}</div>
          <span className={`text-2xl font-semibold tabular-nums ${rightColor ?? 'text-(--text)'}`}>{rightValue}</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex items-start gap-4 py-6 px-4 sm:px-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-(--text) text-3xl font-semibold leading-tight">{t('home.title')}</h1>
      ...
      <div className="p-8">
        <div className="w-full space-y-8">
          ...
          {/* ステータスセクション */}
          <div>
            <h5 className="text-lg font-semibold text-(--text) mb-4">{t('home.statusSection')}</h5>
          ...
          {/* プロジェクト一覧セクション */}
          <div>
            <h5 className="text-lg font-semibold text-(--text) mb-4">{t('home.projectList')}</h5>
            ...
            {loading ? (
              ...
              <div className="rounded-lg bg-(--bg-surface) ring-1 ring-(--border) p-12 text-center">
                <p className="text-(--text-faint) text-sm">{t('home.noProjects')}</p>
              </div>
            ) : (
              <div className="grid gap-4 md:gap-5 grid-cols-1 md:grid-cols-3">
                {projects.map(project => {
                  ...
                  return (
                    <Link key={project.id} to={`/${uuid}/projects/${project.id}`} className="overflow-hidden rounded-lg bg-(--bg-surface) shadow-xs ring-1 ring-(--border) w-full h-full flex flex-col transition-colors hover:ring-(--accent) no-underline text-inherit group outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-(--accent) cursor-pointer">
                        ...
                        <div className="relative flex flex-col overflow-hidden bg-(--bg-surface) group-hover:bg-(--bg-raised) transition-colors flex-1">
                          <div className="grid h-full auto-rows-fr grid-cols-2">
                            <div className="flex min-h-22 flex-col gap-2 px-4 pt-4 pb-4 justify-center border-b border-r border-(--border)">
                              <div className="text-xs font-medium text-(--text-muted)">{t('home.cardDeviceCount')}</div>
                              <span className="text-xl leading-none font-semibold text-(--text)">{devs.length}</span>
                            </div>
                            <div className="flex min-h-22 flex-col gap-2 px-4 pt-4 pb-4 justify-center border-b border-(--border)">
                              <div className="text-xs font-medium text-(--text-muted)">{t('home.cardOnline')}</div>
                              <span className="text-xl leading-none font-semibold text-[#2db35e]">{online}</span>
                            </div>
                            <div className="flex min-h-22 flex-col gap-2 px-4 pt-4 pb-4 justify-center border-r border-(--border)">
                              <div className="text-xs font-medium text-(--text-muted)">{t('home.cardWarning')}</div>
                            ...
                            <div className="flex min-h-22 flex-col gap-2 px-4 pt-4 pb-4 justify-center">
                              <div className="text-xs font-medium text-(--text-muted)">{t('home.cardOffline')}</div>
                              <span className="text-xl leading-none font-semibold text-(--danger-text)">{offline}</span>
                            </div>
                          </div>
