  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="bg-(--bg-surface) ring-1 ring-(--border) rounded-xl w-full max-w-md p-6 shadow-2xl"
        ...
        <h2 className="text-(--text) text-lg font-semibold mb-5">{t('apiTokens.createModal.title')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm text-(--text-dim) mb-1.5">{t('apiTokens.createModal.nameLabel')}</label>
            <input value={name} onChange={e => setName(e.target.value)}
              className={inputClass} />
          </div>
          <div>
            <label className="block text-sm text-(--text-dim) mb-1.5">{t('apiTokens.createModal.typeLabel')}</label>
            <CustomSelect
              value={type}
...
function TokenReveal({ token, onClose }: TokenRevealProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-(--bg-surface) ring-1 ring-(--border) rounded-xl w-full max-w-lg p-6 shadow-2xl">
        <h2 className="text-(--text) text-lg font-semibold mb-2">{t('apiTokens.revealModal.title')}</h2>
        <p className="text-(--text-dim) text-sm mb-4">
          {t('apiTokens.revealModal.body')}
        </p>
        <div className="flex items-center gap-2 bg-(--bg-base) ring-1 ring-(--border) rounded-lg px-3 py-2.5 mb-5">
          <code className="flex-1 text-xs text-green-400 font-mono break-all">{token}</code>
          <button onClick={handleCopy}
            className="shrink-0 h-7 px-3 rounded-md text-xs text-(--text-muted) bg-(--bg-surface) hover:bg-(--bg-subtle)/60 ring-1 ring-(--border) transition-colors cursor-pointer">
            {copied ? t('common.copied') : t('common.copy')}
          </button>
        </div>
        <div className="flex justify-end">
          <button onClick={onClose}
            className="h-9 px-4 rounded-lg text-sm font-medium text-white bg-(--accent) hover:bg-(--accent-hover) transition-colors cursor-pointer">
...
function RevokeConfirm({ token, onClose, onConfirm }: RevokeConfirmProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="bg-(--bg-surface) ring-1 ring-(--border) rounded-xl w-full max-w-md p-6 shadow-2xl"
        onClick={e => e.stopPropagation()}>
        <h2 className="text-(--text) text-lg font-semibold mb-2">{t('apiTokens.revokeModal.title')}</h2>
...
export function ApiTokens() {
  ...
  if (!canEdit) {
    return (
      <div className="flex flex-col min-h-full">
        <div className="p-8">
          <p className="text-(--text-dim) text-sm">{t('apiTokens.accessDenied')}</p>
        </div>
      </div>
    );
  }
  ...
  function TokenCard({ t: tok }: { t: ApiToken }) {
    ...
    return (
      <div className="bg-(--bg-surface) ring-1 ring-(--border) rounded-xl px-4 py-4">
        <div className="flex items-start justify-between gap-3 mb-2">
          <span className={`text-sm font-semibold leading-snug truncate flex-1 ${isRevoked ? 'text-(--text-faint) line-through' : 'text-(--text)'}`}>
            {tok.name}
          </span>
        ...
        <div className="space-y-0.5 mb-3">
          <p className="text-(--text-faint) text-xs">{t('apiTokens.table.issued')}: {formatDate(tok.createdAt)}</p>
          <p className="text-(--text-faint) text-xs">{t('apiTokens.table.lastUsed')}: {formatDate(tok.lastUsedAt)}</p>
        </div>
        <div className="flex items-center justify-between">
          <span className={`text-xs font-medium ${isRevoked ? 'text-(--text-faint)' : 'text-green-400'}`}>
  ...
  function TokenRow({ t: tok }: { t: ApiToken }) {
    const isRevoked = !!tok.revokedAt;
    return (
      <div className="grid grid-cols-[1fr_90px_160px_160px_100px_80px] gap-4 px-4 py-3.5 items-center bg-(--bg-surface) transition-colors border-b border-(--border) last:border-b-0">
        <span className={`text-sm font-medium truncate ${isRevoked ? 'text-(--text-faint) line-through' : 'text-(--text)'}`}>
          {tok.name}
        </span>
        ...
        <span className="text-(--text-dim) text-xs tabular-nums">{formatDate(tok.createdAt)}</span>
        <span className="text-(--text-dim) text-xs tabular-nums">{formatDate(tok.lastUsedAt)}</span>
        <span className={`text-xs font-medium ${isRevoked ? 'text-(--text-faint)' : 'text-green-400'}`}>
          {isRevoked ? t('apiTokens.statusLabel.revoked') : t('apiTokens.statusLabel.active')}
        </span>
        <div className="flex justify-end">
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex items-start gap-2 min-w-0 py-6 px-4 sm:hidden">
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          <h1 className="text-(--text) text-3xl font-semibold leading-tight">{t('apiTokens.title')}</h1>
          <p className="text-(--text-muted) text-base">{t('apiTokens.description')}</p>
        </div>
        {canEdit && (
          <div ref={headerMenuRef} className="relative shrink-0 mt-2">
            <button
              onClick={() => setHeaderMenuOpen(o => !o)}
              className="w-8 h-8 flex items-center justify-center rounded-md text-(--text-dim) hover:text-(--text) hover:bg-(--bg-hover) transition-colors cursor-pointer"
            ...
            {headerMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-(--bg-raised) ring-1 ring-(--border) rounded-lg shadow-xl overflow-hidden z-10">
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-sm text-(--text) hover:bg-(--bg-hover) transition-colors cursor-pointer flex items-center gap-2"
                >
      ...
      <div className="hidden sm:flex items-start justify-between gap-4 py-6 px-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-(--text) text-3xl font-semibold">{t('apiTokens.title')}</h1>
          <p className="text-(--text-muted) text-base">{t('apiTokens.description')}</p>
        </div>
        {canEdit && (
          <button
      ...
      <div className="px-4 sm:px-6 pb-2 flex flex-wrap items-end gap-2">
        <div className="flex flex-col gap-1">
          <label className="text-(--text-faint) text-xs">{t('apiTokens.filterType')}</label>
          <CustomSelect
            value={filterType}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-(--text-faint) text-xs">{t('apiTokens.filterStatus')}</label>
          <CustomSelect
            value={filterStatus}
      ...
      <div className="px-4 sm:px-6 pt-4 pb-8 space-y-6">
        {loading ? (
          <div className="rounded-lg bg-(--bg-surface) ring-1 ring-(--border) p-12 text-center">
          ...
          <>
            {/* 有効なトークン */}
            <div>
              <p className="text-sm font-medium text-(--text-dim) mb-2">{t('apiTokens.active')} <span className="text-(--text-faint)">({active.length})</span></p>
              {active.length === 0 ? (
                <div className="rounded-lg bg-(--bg-surface) ring-1 ring-(--border) p-8 text-center">
                  <p className="text-(--text-faint) text-sm">{t('apiTokens.noActive')}</p>
                </div>
              ) : (
                <>
                  {/* Mobile cards */}
                  ...
                  <div className="hidden sm:block overflow-hidden rounded-lg ring-1 ring-(--border)">
                    <div className="grid grid-cols-[1fr_90px_160px_160px_100px_80px] gap-4 px-4 py-3 bg-(--bg-base) border-b border-(--border) text-xs font-medium text-(--text-faint) uppercase tracking-wider">
            ...
            {revoked.length > 0 && (
              <div>
                <p className="text-sm font-medium text-(--text-dim) mb-2">{t('apiTokens.revoked')} <span className="text-(--text-faint)">({revoked.length})</span></p>
                <>
                  {/* Mobile cards */}
                  <div className="sm:hidden space-y-4">
