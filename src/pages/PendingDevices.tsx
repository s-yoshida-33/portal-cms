      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
        <div className="bg-[#111111] ring-1 ring-[#3d3d3d] rounded-xl w-full max-w-md p-6 shadow-2xl">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-white text-lg font-semibold">{t('pendingDevices.approveModal.doneTitle')}</h2>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="bg-[#111111] ring-1 ring-[#3d3d3d] rounded-xl w-full max-w-md p-6 shadow-2xl"
        ...
        <h2 className="text-white text-lg font-semibold mb-1">{t('pendingDevices.approveModal.title')}</h2>
        <p className="text-zinc-400 text-sm mb-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-[#1a1a1a] ring-1 ring-[#3d3d3d] rounded-lg px-4 py-3 space-y-1.5 text-sm">
            <div className="flex gap-2">
              <span className="text-zinc-500 w-20 shrink-0">{t('pendingDevices.approveModal.appLabel')}</span>
              <span className={`inline-flex items-center h-5 px-2 rounded-full text-xs font-medium ${appBadge[pending.appName]}`}>
                {pending.appName}
              </span>
            </div>
            <div className="flex gap-2">
              <span className="text-zinc-500 w-20 shrink-0">{t('pendingDevices.approveModal.hostnameLabel')}</span>
              <span className="text-zinc-300 font-mono">{pending.hostname}</span>
            </div>
            <div className="flex gap-2">
              <span className="text-zinc-500 w-20 shrink-0">{t('pendingDevices.approveModal.ipLabel')}</span>
          ...
          {/* デバイス名 */}
          <div className="space-y-1.5">
            <label className="text-zinc-400 text-xs font-medium">{t('pendingDevices.approveModal.nameLabel')}</label>
            <input
              type="text"
              value={deviceName}
          ...
          {/* プロジェクト選択 */}
          <div className="space-y-1.5">
            <label className="text-zinc-400 text-xs font-medium">{t('pendingDevices.approveModal.projectLabel')}</label>
            {projects.length === 0 ? (
              <p className="text-zinc-500 text-sm">{t('pendingDevices.approveModal.noProjects')}</p>
            ) : (
              <CustomSelect
                value={projectId}
...
function RejectConfirm({ pending, onClose, onDone }: RejectConfirmProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="bg-[#111111] ring-1 ring-[#3d3d3d] rounded-xl w-full max-w-md p-6 shadow-2xl"
        onClick={e => e.stopPropagation()}>
        <h2 className="text-white text-lg font-semibold mb-2">{t('pendingDevices.rejectModal.title')}</h2>
        <p className="text-zinc-400 text-sm mb-5">
  ...
  if (role !== 'admin' && role !== 'owner') {
    return (
      <div className="flex flex-col min-h-full">
        <div className="p-8">
          <p className="text-zinc-400 text-sm">{t('pendingDevices.accessDenied')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex items-start justify-between gap-4 py-6 px-4 sm:px-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-white text-3xl font-semibold leading-tight">{t('pendingDevices.title')}</h1>
          <p className="text-[#999999] text-base">{t('pendingDevices.description')}</p>
        </div>
      ...
      <div className="px-4 sm:px-6 pt-8 pb-8">
        {loading ? (
          ...
          <div className="overflow-hidden rounded-lg bg-[#111111] ring-1 ring-[#3d3d3d] p-12 text-center">
            <p className="text-zinc-500 text-sm">{t('pendingDevices.noDevices')}</p>
          </div>
        ) : (
          <>
            {/* モバイルカード */}
